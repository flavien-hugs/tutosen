...
class Post(models.Model):
    uuid = models.UUIDField(
        db_index=True, editable=False, default=uuid.uuid4, verbose_name="post ID"
    )
    title = models.CharField(
        verbose_name="title", max_length=255, help_text="add title for article"
    )
    ...
    slug = models.SlugField(verbose_name="post link", max_length=255, unique=True)
    ...
    author = models.ForeignKey(
        verbose_name="author", to=Teacher, on_delete=models.PROTECT
    )
    ...
    def get_post_update(self):
        return reverse(
            ...
            kwargs={"link": str(self.author.link), "slug": str(self.slug)},
        )
