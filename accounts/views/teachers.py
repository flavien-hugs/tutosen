...
def check_validate_data_view(request):
    ...
    response = {
        "email_is_taken": get_user_model()
        .objects.filter(email__iexact=check_email)
        .exists()
    }
...
class TeacherDashboardDetailView(
    LoginRequiredMixin, mixins.GetUser, generic.DetailView
):
    """
...
class TeacherProfileListView(mixins.TeacherSearchMixin, generic.ListView):
    ...
    queryset = models.Teacher.objects.order_by("-date_joined").exclude(
        is_superuser=True
    )
...
class TeacherCourseListView(MultipleObjectMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        object_list = Subject.objects.get_courses_published().filter(
            instructor=self.get_object()
        )
        return super(TeacherCourseListView, self).get_context_data(
            teacher=teacher, page_title=page_title, object_list=object_list, **kwargs
        )
