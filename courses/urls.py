...
urlpatterns = [
    ...
    # course/<slug>/lessons/
    path('<slug>/lesson/', include(([
        path(route="<chapiter_slug>~<pk>", view=course.lesson_detail_view, name='chapiter_detail'),
        path(
            route="<chapiter_slug>~<pk>~<lesson_slug>/", 
            view=course.lesson_detail_view, 
            name='chapiter_lesson_detail'
        ),
    ], 'courses'), namespace='lessons')),
    ...
    ], 'courses'), namespace='student_enrolled'))
]
