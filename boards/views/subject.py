...
class SubjectListView(mixins.TeacherSubjectMixin, generic.ListView):
    ...
    def get_queryset(self):
        subject = self.model.objects.filter(instructor=self.request.user)
        return subject
...
class SubjectCreateView(
    views.SuccessMessageMixin, mixins.TeacherSubjectEditMixin, generic.CreateView
):
    success_message = "Course successfully created !"


subject_create_view = SubjectCreateView.as_view(
    extra_context={"page_title": "create new course"}
)


class SubjectUpdateView(
    views.SuccessMessageMixin, mixins.TeacherSubjectEditMixin, generic.UpdateView
):
    success_message = "Course successfully updated !"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["form"] = SubjectForm(instance=self.get_object())
...
class SubjectDeleteView(
    views.SuccessMessageMixin, mixins.TeacherSubjectEditMixin, generic.DeleteView
):
